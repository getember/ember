...
const devProductionEmber = (process.env.CLIENT_TYPE ?? '') === 'dev-ember'
const devProductionBold = (process.env.CLIENT_TYPE ?? '') === 'dev-bold'
const dev =
  (process.env.CLIENT_TYPE ?? '') === 'dev' || devServer || devProduction || devProductionEmber || devProductionBold
...
module.exports = [
  {
    ...
    entry: {
      serviceWorker: '@digitranslab/notification/src/serviceWorker.ts'
    },
    module: {
      rules: [
        {
  ...
  },
  {
    cache: useCache
      ...
      : undefined,
    entry: {
      bundle: ['@digitranslab/theme/styles/global.scss', ...(dev ? ['./src/main-dev.ts'] : ['./src/main.ts'])]
    },
    ignoreWarnings: [
      {
        message: /a11y-/
    ...
    resolve: {
      ...
      alias: {
        ...
        '@digitranslab/platform-rig/profiles/ui/svelte': path.resolve('node_modules', 'svelte/src/runtime')
      },
      fallback: {
        crypto: false
      },
    ...
    devServer: {
      ...
      proxy:
        devServer && !devProduction && !devProductionEmber && !devProductionBold
          ? {
              '/account': {
          ...
          : !devProductionEmber && !devProductionBold
          ? {
              '/account': {
                target: 'https://account.digitrans.lab/',
                changeOrigin: true,
                pathRewrite: { '^/account': '' },
                logLevel: 'debug'
              },
              '/files': {
                target: 'https://front.digitrans.lab/files',
                changeOrigin: true,
                pathRewrite: { '^/files': '' },
                logLevel: 'debug'
              },
          ...
          ? {
              '/account': {
                target: 'https://account.ember.app/',
                changeOrigin: true,
                pathRewrite: { '^/account': '' },
                logLevel: 'debug'
              },
              '/api/v1': {
                target: 'http://ember.app',
                changeOrigin: true,
                logLevel: 'debug'
              },
              '/files': {
                target: 'https://ember.app/files',
                changeOrigin: true,
                pathRewrite: { '^/files': '' },
                logLevel: 'debug'
              },
              '/rekoni/recognize': {
                target: 'https://rekoni.ember.app',
                changeOrigin: true,
                pathRewrite: { '^/rekoni/recognize': '/recognize' },
                logLevel: 'debug'
              }
