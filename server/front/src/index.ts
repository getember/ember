...
import { MeasureContext, Blob as PlatformBlob, WorkspaceId, metricsAggregate, type Ref } from '@digitranslab/core'
import { Token, decodeToken } from '@digitranslab/server-token'
import { StorageAdapter } from '@digitranslab/storage'
...
export function start (
  ...
  const handleDelete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // TODO: We need to allow delete only of user attached documents. (https://front.digitrans.lab/workbench/platform/tracker/TSK-1081)
