...
export class SidebarPage extends CommonPage {
  ...
  contentHeaderByTitle = (title: string): Locator =>
    this.content().locator(`.emberHeader-titleGroup:has-text("${title}")`)

  contentCloseButton = (): Locator => this.content().locator('.emberHeader-container button.iconOnly').last()
  ...
  plannerSidebarNextDayButton = (): Locator =>
    this.sidebar().locator('.emberHeader-buttonsGroup').getByRole('button').last()
