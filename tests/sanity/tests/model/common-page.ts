...
export class CommonPage {
  ...
  emberPopupRowButton = (name: string): Locator =>
    this.page.locator('div.emberPopup-container button.emberPopup-row', { hasText: name })
  ...
  linesFromTable = (text: string = ''): Locator =>
    this.page.locator('.emberComponent table tbody tr').filter({ hasText: text })

  linesFromList = (text: string = ''): Locator =>
    this.page.locator('.emberComponent .list-container div.row').filter({ hasText: text })
  ...
  buttonBreadcrumb = (hasText?: string): Locator => this.page.locator('button.emberBreadcrumb-container', { hasText })
  ...
  async selectPopupItem (name: string): Promise<void> {
    await this.emberPopupRowButton(name).click({ delay: 100 })
  }
