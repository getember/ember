...
export class SettingsPage extends CommonPage {
  ...
  spaceTypesHeader = (): Locator => this.page.locator('button.emberNavGroup-header', { hasText: 'Space types' })
  ...
  spaceTypeButton = (name: string, category?: string): Locator =>
    this.page.locator('div#navGroup-spaceTypes button.emberTaskNavLink-container', {
      hasText: category !== undefined ? `${name} ${category}` : name
    })

  breadcrumbButton = (hasText: string): Locator => this.page.locator('button.emberBreadcrumb-container', { hasText })
  stateButton = (name: string): Locator =>
    this.page
      .locator('div.emberTableAttr-header', { hasText: 'Process states' })
      .locator('xpath=..')
      .locator('button.emberTableAttr-content__row', { hasText: name })

  statusNameInput = (): Locator =>
    this.page.locator('div.emberModal-container.type-aside', { hasText: 'Edit state' }).getByPlaceholder('Status name')

  selectColorButton = (name: string): Locator =>
    this.page.locator('div.emberTableAttr-container', { hasText: 'Color' }).locator(`div.color[data-id="${name}"]`)
  ...
  taskTypeRow = (value: string): Locator =>
    this.page
      .locator('div.emberTableAttr-header', { hasText: 'Task types' })
      .locator('xpath=..')
      .locator('div.emberTableAttr-content button.emberTableAttr-content__row', { hasText: value })

  addTaskTypeButton = (): Locator =>
    this.page.locator('div.emberTableAttr-header', { hasText: 'Task types' }).locator('button[data-id="btnAdd"]')

  taskNameInput = (): Locator => this.page.getByPlaceholder('Task name *')
  taskTypeButton = (): Locator =>
    this.page.locator('div.emberModal-content__settingsSet-line', { hasText: 'Task type' }).locator('button')

  asideFooterButton = (hasText: string): Locator =>
    this.page.locator('div.emberModal-container.type-aside div.emberModal-footer button', { hasText })

  buttonRoleInComponent = (hasText: string): Locator =>
    this.page.locator('div.emberComponent-content > div.flex-row-center', { hasText }).locator('button')
