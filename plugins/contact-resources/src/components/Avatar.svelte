...
<script lang="ts" context="module">
  import contact, { AvatarProvider, getAvatarColorForId, type AvatarInfo } from '@digitranslab/contact'
  import { Ref, type Data, type WithLookup } from '@digitranslab/core'
  import { getClient, sizeToWidth } from '@digitranslab/presentation'
...
<script lang="ts">
  import { Employee, getAvatarProviderId, getFirstName, getLastName, Person } from '@digitranslab/contact'
  import { Asset, getMetadata, getResource } from '@digitranslab/platform'
  import { getBlobURL, reduceCalls } from '@digitranslab/presentation'
  import {
    AnySvelteComponent,
    ColorDefinition,
    IconSize,
...
{#if showStatus && accounts.length > 0}
  <div class="relative">
    <AvatarInstance
      ...
      withStatus
    />
    <div class="emberAvatar-statusMarker {size}" class:online={isOnline} class:offline={!isOnline} />
  </div>
{:else}
  <AvatarInstance
    bind:this={avatarInst}
