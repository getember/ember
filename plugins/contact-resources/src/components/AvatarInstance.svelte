...
<script lang="ts">
  ...
  import { AnySvelteComponent, ColorDefinition, Icon, IconSize, resizeObserver } from '@digitranslab/ui'
...
{#if size === 'full' && !url && displayName && displayName !== ''}
  <div
    bind:this={element}
    class="emberAvatar-container emberAvatarSize-{size} {variant}"
...
{:else}
  <div
    bind:this={element}
    class="emberAvatar-container emberAvatarSize-{size} stat {variant}"
    class:no-img={!hasImg && color}
    class:bordered={!hasImg && color === undefined}
    class:border={bColor !== undefined}
    class:withStatus
  ...
  >
    {#if url && !imgError}
      <img class="emberAvatarSize-{size} ava-image" src={url} {srcset} alt={''} on:error={handleImgError} />
