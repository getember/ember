...
<script lang="ts">
  ...
  import { Avatar, CombineAvatars, personByIdStore } from '@digitranslab/contact-resources'
  import { Ref } from '@digitranslab/core'
  import { getClient } from '@digitranslab/presentation'
  import { Icon, IconSize } from '@digitranslab/ui'
  import { classIcon } from '@digitranslab/view-resources'
...
{#if persons.length > 1 && size !== 'medium'}
  {#if compact}
    <div class="emberAvatar-container emberAvatarSize-{size} group-ava">
      {persons.length}
    </div>
  {:else}
    <CombineAvatars
