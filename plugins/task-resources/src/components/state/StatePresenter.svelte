...
-->
<script lang="ts">
  import core, { IdMap, Ref, Status, StatusCategory } from '@digitranslab/core'
  import { Asset } from '@digitranslab/platform'
  import { getClient, reduceCalls } from '@digitranslab/presentation'
  import task, { Project, ProjectType, TaskType } from '@digitranslab/task'
  import {
    ColorDefinition,
    Icon,
    IconSize,
...
{#if value}
  {#if kind === 'table-attrs'}
    <button class="emberTableAttr-content__row-icon-wrapper" on:click>
      {#if icon != null}
    ...
    <span
      class="emberTableAttr-content__row-label font-medium-12 uppercase grow overflow-label"
