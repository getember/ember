...
-->
<script lang="ts">
  import { SortingOrder, WithLookup } from '@digitranslab/core'
  import { createQuery, getClient } from '@digitranslab/presentation'
  import tags from '@digitranslab/tags'
  import { Component, IconMoreV2, Spinner, showPanel, Icon } from '@digitranslab/ui'
  import { showMenu } from '@digitranslab/view-resources'
  import time, { ToDo, ToDoPriority, WorkSlot } from '@digitranslab/time'
...
>
  <div class="flex-row-center flex-grow flex-gap-2">
    <div class="emberToDoLine-statusPriority" class:isNew>
      {#if updating !== undefined}
        <Spinner size={'small'} />
      {:else}
        <button class="emberToDoLine-dragbox" on:contextmenu={onMenuClick}>
    ...
    <div class="emberToDoLine-title text-left font-regular-14 line-height-auto overflow-label">
      {todo.title}
    </div>
  </div>
  ...
  <div class="emberToDoLine-checkbox" class:updating>
