...
-->
<script lang="ts">
  import { LoginInfo, WorkspaceLoginInfo } from '@digitranslab/login'
  import { getAccount, getWorkspaces, navigateToWorkspace } from '@digitranslab/login-resources'
  import { OK } from '@digitranslab/platform'
  ...
  const action = {
    i18n: onboard.string.StartUsingEmber,
    func: async () => {
      if (account !== undefined && isWorkspaceLoginInfo(account)) {
        Analytics.handleEvent(OnboardEvents.StartEmber)
