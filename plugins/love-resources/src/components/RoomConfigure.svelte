...
-->
<script lang="ts">
  import contact, { Contact, Person } from '@digitranslab/contact'
  import { AssigneeBox, personByIdStore } from '@digitranslab/contact-resources'
  import { Ref } from '@digitranslab/core'
  import { getClient } from '@digitranslab/presentation'
  import { ActionIcon, EditBox, Icon, IconDelete, resizeObserver } from '@digitranslab/ui'
  import { Room, RoomAccess, RoomType, isOffice } from '@digitranslab/love'
...
<div
  ...
  class:placed
  style={dragStyle}
  style:--ember-floor-roomWidth={room.width}
  style:--ember-floor-roomHeight={room.height}
  style:--ember-floor-roomShadow={`var(--theme-popup-shadow), ${$shadow.x}px ${$shadow.y}px ${$shadow.r}px ${$shadow.s}px rgba(${$shadowColor.r}, ${$shadowColor.g}, ${$shadowColor.b}, ${$shadowColor.a})`}
