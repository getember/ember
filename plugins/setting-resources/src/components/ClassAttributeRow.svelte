...
-->
<script lang="ts">
  import core, { AnyAttribute, ArrOf, Doc, EnumOf, RefTo, Type } from '@digitranslab/core'
  import { IntlString } from '@digitranslab/platform'
  import { getClient } from '@digitranslab/presentation'
  import { AnySvelteComponent, Icon, IconMoreV2, Label, IconOpenedArrow } from '@digitranslab/ui'
...
<button class="emberTableAttr-content__row" class:hovered class:selected on:contextmenu on:click>
  <button class="emberTableAttr-content__row-dragMenu" on:click|stopPropagation={clickMore}>
    <IconMoreV2 size={'small'} />
  </button>
  {#if attribute.isCustom}
    <div class="emberChip-item font-medium-12">
      <Label label={settings.string.Custom} />
    </div>
  {/if}
  {#if attribute.icon !== undefined}
    <div class="emberTableAttr-content__row-icon">
      <Icon icon={attribute.icon} size={'small'} />
    </div>
  {/if}
  <div class="emberTableAttr-content__row-label font-regular-14 grow" class:accent={!attribute.hidden}>
    <Label label={attribute.label} />
  </div>
  {#if attributeMapper}
    <svelte:component this={attributeMapper.component} {...attributeMapper.props} {attribute} />
  {/if}
  <div class="emberTableAttr-content__row-type font-medium-12">
