...
import { Account, BrandingMap, MeasureContext, Ref } from '@digitranslab/core'
...
export async function start (ctx: MeasureContext, brandingMap: BrandingMap): Promise<() => Promise<void>> {
  ...
  ctx.info('Running Ember Github integration', { appId: config.AppID, clientID: config.ClientID })
