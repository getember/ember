...
export const SubLink = Node.create<SubLinkOptions>({
  ...
  addOptions () {
    return {
      HTMLAttributes: {},
      hasEmberText: (text: string) => false,
      hasEmberLink: (href: string) => false
    }
  },
    ...
    return [
      {
        ...
        getAttrs: (el: HTMLElement | string) => {
          if (typeof el !== 'string') {
            if (this.options.hasEmberText(el.textContent ?? '')) {
              return null
            }

            const link = el.querySelector('a[href]')
            const href = link?.getAttribute('href') ?? ''
            if (link != null && this.options.hasEmberLink(href)) {
              return null
            }
          }
          // no match
