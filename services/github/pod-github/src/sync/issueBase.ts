...
import { CollaboratorClient } from '@digitranslab/collaborator-client'
...
import { LiveQuery } from '@digitranslab/query'
import task, { TaskType } from '@digitranslab/task'
import { MarkupNode, MarkupNodeType, areEqualMarkups, markupToJSON, traverseNode } from '@digitranslab/text'
import time, { type ToDo } from '@digitranslab/time'
import tracker, { Issue, IssuePriority } from '@digitranslab/tracker'
  ...
  async collectIssueUpdate (
    ...
    if (platformUpdate.description != null) {
      ...
      issueUpdate.body = await this.provider.getMarkdown(
        ...
        info.allowOpenInEmber === true ? pp : undefined
      )
