...
const sessionPartition = !isDev ? 'persist:ember' : 'persist:ember_dev'
...
function readServerUrl (): string {
  ...
  return ((settings as any).get('server', process.env.FRONT_URL) as string) ?? 'https://ember.app'
}
...
console.log('Running Ember', process.env.MODEL_VERSION, process.env.VERSION, isMac, isDev, process.env.NODE_ENV)
...
ipcMain.on('set-combined-config', (event, config: Config) => {
  ...
  const updatesUrl = process.env.DESKTOP_UPDATES_URL ?? config.DESKTOP_UPDATES_URL ?? 'https://dist.ember.io'
  const updatesChannel = process.env.DESKTOP_UPDATES_CHANNEL ?? config.DESKTOP_UPDATES_CHANNEL ?? 'ember'
...
const DEEP_LINKS_PROTOCOL = 'ember'
/*
