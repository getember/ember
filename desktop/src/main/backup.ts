...
import { getWorkspaceId, MeasureMetricsContext, type BackupClient, type Client } from '@digitranslab/core'
import { addLocation, getResource, setMetadata } from '@digitranslab/platform'
...
export async function createClient (
  ...
  addLocation(clientId, () => import('@digitranslab/client-resources'))
...
export function startBackup (window: BrowserWindow, token: string, endpoint: string, workspace: string, cmd: (command: string, ...args: any[]) => void): void {
  void dialog
    .showOpenDialog(window, {
      ...
      message: 'Select a folder for Ember incremental backup.'
    })
    .then((response) => {
      if (!response.canceled && response.filePaths.length > 0) {
        void dialog
          .showMessageBox({
            ...
            defaultId: 0,
            textWidth: 500,
            title: 'Ember desktop need a confirmation for a backup process to be started...',
            message: `Ember Desktop need a confirmation for a backup process to be started....\n\n
