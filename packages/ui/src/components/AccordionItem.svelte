...
<div class="emberAccordionItem-container {kind}" class:nested>
  <button
    class="emberAccordionItem-header {kind} {size}"
    class:bottomSpace
    class:nested
    class:disabled
  ...
  >
    <div
      class="emberAccordionItem-header__label-wrapper {size === 'large' ? 'heading-medium-16' : 'font-medium-12'}"
      class:withIcon={size === 'medium' && icon !== undefined}
    >
      {#if size === 'large' && !disabled}
        <div class="emberAccordionItem-header__chevron">
      ...
      <div
        class="emberAccordionItem-header__label"
      ...
      {#if counter !== false && ($$slots.counter || typeof counter === 'number')}
        <span class="emberAccordionItem-header__separator">•</span>
        <span class="emberAccordionItem-header__counter">
      ...
      {#if duration !== false && ($$slots.duration || duration !== 0)}
        <span class="emberAccordionItem-header__separator">•</span>
        <span class="emberAccordionItem-header__duration">
          {#if typeof duration === 'number' && durationLabel !== ''}{durationLabel}{/if}
          <slot name="duration" />
        </span>
      {/if}
    </div>
    <div class="emberAccordionItem-header__tools">
      <slot name="actions" />
    </div>
  </button>
  <div class="emberAccordionItem-content">
    <slot />
  </div>
</div>
