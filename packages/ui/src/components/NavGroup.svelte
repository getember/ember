...
>
  ...
  >
    {#if isFold && !empty}
      <button class="emberNavGroup-header__chevron" class:collapsed={!isOpen}>
        <IconDown size={'small'} />
      </button>
    {/if}
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <div class="emberNavGroup-header__label font-medium-12" on:click={handleMenuClicked}>
      {#if visibleIcon}
        <div class="emberNavGroup-header__icon" class:folder={folderIcon}>
    ...
    {#if tools || $$slots.tools}
      <div class="emberNavGroup-header__tools">
        {#if tools}
          <Component
            is={tools}
            props={{
    ...
    {#if selected && type === 'nested-selectable'}
      <div class="emberNavGroup-header__arrow"><IconOpenedArrow size={'small'} /></div>
    {/if}
  </button>
  {#if !empty}
    <div {id} class="emberNavGroup-content">
