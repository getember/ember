...
>
  {#if _doubleRow}
    <div class="emberHeader-row">
      {#if allowFullsize}
        ...
        />
        <div class="emberHeader-divider no-print" />
      {/if}
      {#if $$slots.beforeTitle && !hideBefore}
        <div class="emberHeader-buttonsGroup before mr-2 no-print" class:freezeBefore>
      ...
      >
        {#if $$slots.description && !hideDescription}
          <div class="emberHeader-titleGroup"><slot /></div>
          <div class="emberHeader-titleGroup"><slot name="description" /></div>
        {:else}
          <slot />
        {/if}
      </div>
      ...
      {#if $$slots.actions && !hideActions && (adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="emberHeader-buttonsGroup actions no-print">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
      {#if closeButton}
        {#if type !== 'type-popup'}<div class="emberHeader-divider no-print" />{/if}
        {#if closeOnEscape}
          <div class="emberHotKey-item no-print">Esc</div>
        {/if}
        <ButtonIcon icon={IconClose} kind={'tertiary'} size={'small'} noPrint on:click={() => dispatch('close')} />
      {/if}
    </div>
    <!-- <div class="emberHeader-row__divider" /> -->
    <div class="emberHeader-row no-print" class:between={$$slots.search} class:reverse={!$$slots.search}>
      {#if $$slots.search}
        <div class="emberHeader-buttonsGroup search">
          <slot name="search" {doubleRow} />
        </div>
      {/if}
      {#if $$slots.extra && !hideExtra && (adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div
          class="emberHeader-buttonsGroup extra"
      ...
      {/if}
      {#if $$slots.actions && !hideActions && !(adaptive === 'freezeActions' || adaptive === 'doubleRow' || adaptive === 'autoExtra')}
        <div class="emberHeader-buttonsGroup actions">
          <slot name="actions" {doubleRow} />
        </div>
      {/if}
    </div>
  {:else}
    ...
    {/if}
    {#if $$slots.beforeTitle && !hideBefore}
      <div class="emberHeader-buttonsGroup before mr-2 no-print">
        <slot name="beforeTitle" {doubleRow} />
      </div>
    {/if}
    <!-- svelte-ignore a11y-click-events-have-key-events -->
    ...
    {#if $$slots.search && !hideSearch}
      <div class="emberHeader-buttonsGroup search no-print">
        <slot name="search" {doubleRow} />
      </div>
      {#if $$slots.actions && !hideActions}<div class="emberHeader-divider no-print" />{/if}
    {/if}
