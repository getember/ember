...
<button class="emberBreadcrumb-container {size}" class:current={isCurrent} on:click>
  {#if size === 'large' && icon}
    <div
      class="emberBreadcrumb-avatar"
  ...
  {/if}
  <span class="{size === 'large' ? 'heading-medium-16' : 'font-regular-14'} emberBreadcrumb-label overflow-label">
...
<style lang="scss">
  .emberBreadcrumb-container {
    ...
    .emberBreadcrumb-label {
      padding: var(--spacing-0_5) 0;
      color: var(--global-secondary-TextColor);
    }
    &.current .emberBreadcrumb-label {
      font-weight: 700;
    }
    &:not(.current) {
      cursor: pointer;
