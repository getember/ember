...
import contact, { type Person, type PersonAccount } from '@digitranslab/contact'
import { type Class, type Doc, generateId, type Ref, type Space, type TxOperations } from '@digitranslab/core'
import document, { type Document } from '@digitranslab/document'
import { MarkupMarkType, type MarkupNode, MarkupNodeType, traverseNode, traverseNodeMarks } from '@digitranslab/text'
import tracker, { type Issue, Project } from '@digitranslab/tracker'
...
class EmberMarkdownPreprocessor extends BaseMarkdownPreprocessor {
  constructor (
...
export class UnifiedFormatImporter {
  ...
  async importFolder (folderPath: string): Promise<void> {
    ...
    const preprocessor = new EmberMarkdownPreprocessor(
